...
describe('Test Script Runner API Calls for Error Codes', () => {
    ...
    it('Test Error Code 423 for Creating Order', () => {
        cy.apiRequest(methods.post, 'orders', createOrderBody('1234'), null, false).then((res) => {
    ...
    it('Test Error Code 404 for Deleting Order', () => {
        cy.apiRequest(methods.post, 'deliveries', createDeliveryBody(deliveryDate), true).then((res) => {
            ...
            cy.apiRequest(methods.post, 'orders', createOrderBody(deliveryId), true).then((res) => {
                orderId = res.body.data.orderId;
                cy.apiRequest(methods.delete, `orders/${orderId}`, null, true).then(() => {
    ...
    it('Test Error Code 404 for Deleting Delivery', () => {
        cy.apiRequest(methods.post, `deliveries`, createDeliveryBody(deliveryDate), true).then((res) => {
            ...
            cy.apiRequest(methods.post, `orders`, createOrderBody(deliveryId), true).then((res) => {
                orderId = res.body.data.orderId;
                cy.apiRequest(methods.delete, `deliveries/${deliveryId}`, null, true).then(() => {
